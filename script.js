...
$(document).ready(function ($) {
  ...
  $('#information').text('Start the game as soon as you are ready.');
  ...
  $('#dice-launch').on('click', 'span', function () {
    if (reroll_number !== 0) {
      ...
      var image =
        '<img alt="' +
        rand_dice +
        '" style="width: 50px; margin: 5px;" src="images/dice' +
        rand_dice +
        '.png"/>';
      ...
      if (reroll_number === 0) {
        $('#information').text(
          'You have re-rolled as many times as possible, click "end turn" to end your turn.'
        );
      } else {
        $('#information').text(
          'You can still re-roll some dice by clicking on them. (' +
            reroll_number +
            ' remaining)'
        );
      }
  ...
  $('#start-stop').click(function () {
    if (game_number < 2) {
      ...
      game_number++;
      if (game_number === 1) {
        if ($('#yahtzee').text() !== '0') yahtzee_counter++;
        $(this).attr('value', 'End turn');
      } else {
      ...
      $('#information').text(
        'You can now re-roll some dice by clicking on them. (' +
          reroll_number +
          ' remaining)'
      );
...
function updateDicesResults() {
  ...
  for (var i = 1; i < dices.length; ++i) {
    ...
    if (dices[i] !== 0) $('#diceres' + i).text(score.toString());
    else $('#diceres' + i).text('0');
  }
  $('#total').text(total);
}
...
function getScore() {
  ...
  if ($('#yahtzee').text() !== '0')
    return parseInt($('#yahtzee').text()) + bonus;
  if ($('#petite-suite').text() !== '0')
    return parseInt($('#petite-suite').text()) + bonus;
  if ($('#grande-suite').text() !== '0')
    return parseInt($('#grande-suite').text()) + bonus;
...
function updateComboResults() {
  ...
  if (full === 0) $('#brelan').text(brelan.toString());
  else $('#brelan').text('0');
...
function numberOfSameDices(except, number) {
  var dice = -1;
  for (var i = 1; i < dices.length; ++i) {
    if (dices[i] === number && i !== except) dice = i;
  }
  return dice;
}
...
function resetDicesArray() {
  for (var i = 0; i <= 6; ++i) dices.push(0);
}
